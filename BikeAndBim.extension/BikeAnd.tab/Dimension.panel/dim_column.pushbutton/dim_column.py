...
def get_selected_by_cat(categories):
    ...
    if selections_id:
        ...
        logging.debug('Get selected by category {}'.format(len(collector)))
        return collector
...
class AxlesColumn:
    ...
    def get_axles(self):
        axis = self._get_axis()
        logging.debug('#{} Column. Get {} axles'.format(self.column.Id, len(axis)))
        return axis
    ...
    def _get_axis(self):
        ...
        for axis_name in axles_names:
            try:
                ...
                logging.error('Can not found axles with name: "{}"'.format(axis_name))
...
def create_dim_for_column(column, k_offset=4, k_space=1, k_shift_space=0.8):
    ...
    logging.debug('#{} Column. Create {} dimension'.format(column.Id, len(dims)))
